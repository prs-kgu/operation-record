...
function initFooters() {
    const footers = document.querySelectorAll('.system-footer');
    const systemName = "Surgical Report System v5.1";

    footers.forEach(footer => {
        const pageNum = footer.getAttribute('data-page').padStart(2, '0');
        footer.className = "flex justify-between items-center text-[9px] text-slate-400 pt-3 border-t border-slate-100 mt-4";
        footer.innerHTML = `
            <div>Generated by ${systemName}</div>
            <div class="italic">Page ${pageNum}</div>
        `;
    });
}
